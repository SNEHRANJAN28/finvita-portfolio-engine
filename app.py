...
def run_local_portfolio_engine(request: PortfolioRequest):
    ...
    try:
        ...
        # Phase B: Enforcing native structured schema mapping over natural text to output
        # clean mathematical vector objects ready for quantitative analysis.
        ...
        print("📦 Step 2B: Running Structured JSON Transformation...")
        parsing_prompt = f"""
        ...
        for item in market_data_json["items"]:
            ...
            # Formula: Forecasted Return adjusted by half the Sentiment Score factor
            adjusted_returns.append(item["forecasted_return"] + (0.5 * item["sentiment_score"]))
            
    # STEP 3: RESILIENT LOCAL FALLBACK MATRIX
    # Automatically intercepts network timeouts, quota limits (429/503), or formatting anomalies 
    # by swapping in local, risk-matched baseline parameters to ensure zero service disruption.
    except Exception as e:
        print(f"⚠️ Search/Parsing failed ({str(e)}). Engaging local fallback data matrix...")
        using_fallback = True
    ...
    # STEP 4: MATHEMATICAL PORTFOLIO OPTIMIZATION CORE
    # Applies a full Markowitz Mean-Variance framework utilizing a localized covariance historical matrix.
    # Runs an SLSQP solver optimizing constraints: total weight bounds (0-1) and full capital use (Sum of weights = 1).
    print("🧮 Step 3: Executing Markowitz Mean-Variance Optimization Core...")
    ...
    R_f = 0.065 # Risk-Free Rate baseline matching the Indian 10-Year Government Bond (G-Sec) yield
    ...
    # Core Objective: Minimize the Negative Sharpe Ratio to solve for optimal efficiency frontier allocations
    ...
    # STEP 5: OUTPUT STANDARDIZATION & PAYLOAD SEPARATION
    # Isolates structural numerical array components into a separate payload mapping to feed 
    # UI components easily, while building a unified explanatory markdown portfolio text block.
    ...
    if not using_fallback:
        try:
            print("✍️ Step 5A: Generating Dynamic Explainable Advisory Report via Gemini...")
    ...
    if using_fallback:
        print("✍️ Step 5B: Compiling Deterministic Advisory Text Report Structure (Fallback Path)...")
...
if __name__ == "__main__":
    # Local Request Initialization
    sample_request = PortfolioRequest(
        amount=250000.0,
        horizon=7,
        risk_tolerance="high",
        savings_to_income_ratio=0.35,
        primary_investment_goal="wealth_accumulation",
        preferred_rebalancing_frequency="semi_annually"
    )
    
    # Execute structural pipeline
    json_result = run_local_portfolio_engine(sample_request)
    
    print("\n📜 ADVISORY TEXT REPORT:")
    print(json_result["report"])
    
    print("\n📊 FRONTEND STRUCTURAL METRICS ARRAY (`allocations`):")
    print(json.dumps(json_result["allocations"], indent=4))
